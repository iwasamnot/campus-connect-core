...
function loadUniversityData() {
  ...
  try {
    ...
    console.log(`✅ Loaded ${data.length} documents from universityData.json`);
    return data;
  } catch (error) {
    console.error(`❌ Failed to load data from ${dataPath}:`, error.message);
    process.exit(1);
  }
}
...
async function processDocuments(genAI, documents, batchSize = 5) {
  ...
  console.log(`\n📊 Processing ${documents.length} documents in ${totalBatches} batches...`);
  ...
  for (let i = 0; i < documents.length; i += batchSize) {
    ...
    for (const doc of batch) {
      try {
        ...
        console.error(`   ⚠️ Failed to process document ${doc.id}: ${error.message}`);
  ...
  console.log(`✅ Successfully processed ${processedDocuments.length}/${documents.length} documents`);
...
async function upsertToPinecone(pinecone, vectors, batchSize = 100) {
  ...
  console.log(`\n📤 Upserting ${vectors.length} vectors to Pinecone in ${totalBatches} batches...`);
  ...
  console.log(`✅ Total upserted: ${totalUpserted} vectors`);
 ...
 */
async function main() {
  console.log('🚀 Serverless Vector RAG Engine - Data Ingestion');
  console.log('================================================\n');
  ...
  if (vectors.length === 0) {
    console.error('❌ No vectors to upsert. Exiting.');
    process.exit(1);
  }
